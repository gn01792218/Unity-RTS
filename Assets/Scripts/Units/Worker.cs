using UnityEngine;
using UnityEngine.AI;
using UnityEngine.Rendering.Universal;

//標示需要NavMeshAgent組件
//添加了之後，我們就不需要做一大堆的null檢查了
[RequireComponent(typeof(NavMeshAgent))]
public class Worker : MonoBehaviour , ISelectable
{
   
    [SerializeField] Transform target; // The target Transform that the NavMeshAgent will navigate towards
    [SerializeField] private DecalProjector onSelectDecal; // 被選中時的標籤貼紙
    private Vector3 lastTargetPosition; // Cache for the last known target position
    private NavMeshAgent agent; //獲取NavMeshAgent組件
    void Awake()
    {
        // Initialization code here
        agent = GetComponent<NavMeshAgent>();
        onSelectDecal.gameObject.SetActive(false); // Disable the decal projector initially
    }

    void Update()
    {
        if (target)
        {
            if (target.position != lastTargetPosition) // Check if the target's position has changed
            {
                lastTargetPosition = target.position; // Update the cached position
                agent.SetDestination(lastTargetPosition); //設置目標位置
            }
        }
    }

     // Worker class that implements ISelectable interface for unit selection and movement
    public void OnSelect()
    {
        // Handle selection logic here
        //並且設置為選中狀態
        onSelectDecal.gameObject.SetActive(true); // Enable the decal projector
    }

    public void OnDeselect()
    {
        // Handle deselection logic here
        onSelectDecal.gameObject.SetActive(false); // Disable the decal projector when deselected
    }
}
